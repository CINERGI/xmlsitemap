 ...
 -->
    ...
    "@type":"DataCatalog",&#10;</xsl:text>
        ...
        <xsl:if test="$hasSpatial">
            ...
            <!-- check if there are multiple extents that have geographic content
            NOTE: gmd:EX_SpatialTemporalExtent is not handled by this transform!-->

            <!-- logic for counting extents that either have a gmd:geographicElement XOR have
            a gmd:EX_SpatialTemporalExtent (but no geographicElement); there MUST be a 
            gmd:EX_SpatialTemporalExtent/gmd:spatialExtent to be schema valid 
            
  //gmd:extent[ count(descendant::gmd:EX_SpatialTemporalExtent
		[not(parent::*/following-sibling::gmd:geographicElement 
			or parent::*/preceding-sibling::gmd:geographicElement )])>0 
	or count(descendant::gmd:geographicElement)>0]
            
            each gmd:EX_SpatialTemporalExtent/gmd:spatialExtent would need to be processed to extract the geometry -->
            ...
            <xsl:for-each
                ...
                <xsl:for-each select="*//gmd:geographicElement/gmd:EX_GeographicBoundingBox">
                    ...
                    <!--<xsl:if
                        test="parent::*/following-sibling::gmd:geographicElement[count(descendant::gmd:westBoundLongitude) > 0]">
                        <xsl:text>,&#10;            </xsl:text>
                    </xsl:if>-->
                    <xsl:if test="position() != last()">
                        <xsl:text>,&#10;            </xsl:text>
...
<!--    <xsl:template match="gmd:geographicElement">
        ...
        <xsl:variable name="placeIdentifier" select="*//gmd:geographicIdentifier"/>
        ...
        <xsl:variable name="geoPoint">
            ...
            <xsl:if
                test="*//gmd:polygon/*[local-name() = 'Point']/*[(local-name() = 'coordinates') or (local-name() = 'pos')]">
                <xsl:text>      "longitude":"</xsl:text>
                <xsl:value-of
                    select="substring-after(string(//gmd:polygon/*[local-name() = 'Point']/*[(local-name() = 'coordinates') or (local-name() = 'pos')]), ' ')"/>
                <xsl:text>",&#10;</xsl:text>
                <xsl:text>      "latitude":"</xsl:text>
                <xsl:value-of
                    select="substring-before(string(//gmd:polygon/*[local-name() = 'Point']/*[(local-name() = 'coordinates') or (local-name() = 'pos')]), ' ')"/>
                <xsl:text>"</xsl:text>
            </xsl:if>
        </xsl:variable>

        <xsl:if test="string-length(concat($placeIdentifier, $geoPoint, $geoShapeBox)) > 0">
            <xsl:text>{&#10;</xsl:text>
            <xsl:text>"@type": "Place",&#10;</xsl:text>
            ...
            <xsl:if test="string-length($placeIdentifier) > 0">
                <xsl:text>      "identifier": {&#10;</xsl:text>
                <xsl:text>        "@type": "PropertyValue",&#10;</xsl:text>
                <xsl:text>        "propertyID":"</xsl:text>
                <xsl:choose>
                    <xsl:when
                        test="string-length(normalize-space($placeIdentifier//gmd:authority//gmd:title/gco:CharacterString)) > 0">
                        <xsl:value-of
                            select="normalize-space($placeIdentifier//gmd:authority//gmd:title/gco:CharacterString)"
                        />
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:text>location identifier</xsl:text>
                    </xsl:otherwise>
                </xsl:choose>
                <xsl:text>",&#10;</xsl:text>
                <xsl:text>          "value":"</xsl:text>
                <xsl:value-of
                    select="normalize-space($placeIdentifier//gmd:code/gco:CharacterString)"/>
                <xsl:text>"</xsl:text>
                <xsl:text>}</xsl:text>
            </xsl:if>
            ...
            <xsl:if
                test="
                    ...
                    (string-length($placeIdentifier) + string-length(*//gmd:description[1]) > 0)">
                <xsl:text>,&#10;</xsl:text>
            </xsl:if>
